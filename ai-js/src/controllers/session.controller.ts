...
export async function sessionController(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: ChatNewSessionRequest }>(
    ...
    async (
      request: FastifyRequest<{ Body: ChatNewSessionRequest }>,
      reply: FastifyReply,
    ) => {
      const userName = request.user!.username;
      ...
      if (!result.success) {
        return reply.send({
          code: result.code,
          message: StatusMessage[result.code],
        });
      }
  ...
  app.post<{ Body: ChatSendRequest }>(
    ...
    async (
      request: FastifyRequest<{ Body: ChatSendRequest }>,
      reply: FastifyReply,
    ) => {
      const userName = request.user!.username;
    ...
    async (
      ...
      try {
        ...
        if (response !== null) {
          ...
          reply.raw.write(
            `data: ${JSON.stringify({ error: "Session not found or access denied" })}\n\n`,
          );
        }
      } catch (error) {
        reply.raw.write(
  ...
  app.post<{ Body: ChatHistoryRequest }>(
    ...
    async (
      request: FastifyRequest<{ Body: ChatHistoryRequest }>,
      reply: FastifyReply,
    ) => {
      const userName = request.user!.username;
