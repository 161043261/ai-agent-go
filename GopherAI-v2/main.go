...
func main() {
	...
	//初始化缓存（Redis 或 BigCache）
	if err := cache.Init(); err != nil {
		log.Println("Cache init error: " + err.Error())
		return
	}

	// 初始化 Redis 兼容层（供 RAG 等模块使用）
	redis.Init()

	//初始化消息队列
	if err := cache.InitMessageQueue(); err != nil {
		log.Println("InitMessageQueue error: " + err.Error())
		return
	}

	//启动消息消费者
	cache.StartMessageConsumer()
	log.Printf("消息队列初始化成功 [模式: %s]", cache.GetCacheType())
